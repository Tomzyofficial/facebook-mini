...
export async function GET() {
  ...
  try {
    ...
    if (queryResult.rows.length > 0) {
      return new Response(JSON.stringify({ authenticated: true, others: queryResult.rows }), { status: 200 });
    } else {
      return new Response(JSON.stringify({ authenticated: false, others: [] }), { status: 404 });
    }
  } catch (error) {
